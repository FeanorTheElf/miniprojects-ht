...
\end{prop}
\begin{proof}
    Everything we have to show is that $\psi$ maps into $E'$.
    Have
    \begin{align*}
        \psi_y^2 + (v + 1)& \psi_y \psi_x + v \psi_y - \psi_3^3 - v \psi_x^2 \\
        =& (u^3 y + u^2 s x + t)^2 + (v + 1) (u^3 y + u^2 s x + t) (u^2 x) \\
            & + v (u^3 y + u^2 s x + t) - (u^2 x)^3 - v (u^2 x)^2 \\
        =& u^6 y^2 + u^4 s^2 x^2 + t^2 + 2 u^5 s x y + 2 u^3 y t + 2 u^2 s t x \\
            & + (v + 1) u^5 x y + (v + 1) u^4 x^2 + (v + 1) u^2 x t \\
            & + v u^3 y + v u^2 s x + v t - u^6 x^3 - v u^4 x^2 \\
        =& u^6 \Bigl( y^2 + \frac 1 u ( 2 s + 1 + v ) x y + \frac 1 {u^3} (2t + v) y \\
            & - x^3 - \frac 1 {u^2} ()
         \Bigr)
    \end{align*}
