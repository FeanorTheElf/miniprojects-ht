...
\begin{prop}
    ...
    \begin{align*}
        p = 3 \ \Rightarrow \ n^3 + f_2 n^2 + f_1 n + f_0 &\equiv 2 \mod 3 \\
        p = 5 \ \Rightarrow \ n^3 + f_2 n^2 + f_1 n + f_0 &\equiv 2, 3 \mod 5 \\
        p = 7 \ \Rightarrow \ n^3 + f_2 n^2 + f_1 n + f_0 &\equiv 3, 5, 6 \mod 7
    \end{align*}
    for all $n \in \Z$.
...
\begin{proof}
    ...
    \end{align*}
    These cases have been excluded by assumption.
...
Using a computer, it is easy to see that the linear equations
\begin{equation*}
    \forall n \in \F_7: \ f_0 + f_1 n + f_2 n^2 + n^3 \equiv 3, 5, 6 \mod 7
\end{equation*}
...
So instead consider a strengthening of Proposition~\ref{prop:p_adic_points_techniques} part four.
\begin{prop}
    Let $p \geq 5$ be a prime and $E: y^2 = x^3 + f_2 x^2 + f_1 x + f_0$ an Elliptic Curve with $f_0, f_1, f_2 \in \Z$.
...
\begin{proof}
    ...
    Using a computer, it is easy to check that the claim holds for $p = 5$ and $p = 7$ (there are only $5^5$ resp. $7^7$ different combinations of $f_0, f_1, f_2, x, y \in \F_5$ resp. $\F_7$).

    So assume now that $p \divides \Delta(E)$, hence $x^3 + f_2 x^2 + f_1 x + f_0$ factors as
    \begin{equation*}
        x^3 + f_2 x^2 + f_1 x + f_0 \equiv (x - \alpha)^2(x - \beta) \mod p
    \end{equation*}
