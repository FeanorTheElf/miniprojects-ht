...
In this Miniproject, we study several phenomena that can occur when computing the group of rational points $E(\Q)$ of an Elliptic Curve $E$.
In particular, we look at the two most important tools for studying this group that were introduced in the lecture. 
Firstly, this is the fact that the reduction modulo a prime is injective on torsion points, and thus provides a good way to find the torsion part of $E(\Q)$.
The second tool is the 2-descent method, which can be used to find the rank (i.e. the dimension of the free part of $E(\Q)$).

When applying these methods, there are some exceptional (and therefore interesting) situations that can occur.
We want to present some of these phenomena by giving examples, and analyze what conditions must be satisfied for them to occur.
...
Note that in general this is not a group.
...
\begin{proof}
    ...
    \end{equation*}
    Thus $|f(y)|_p < |f'(y)|_p^2$ and Hensel's Lemma \cite[Thm 2.14]{lecture} yields a root $\gamma \in \Z_p$ with $(x, \gamma) \in E(\Z_p)$.
...
\begin{proof}
    ...
    \end{equation*}
    Hensel's Lemma \cite[Thm 2.14]{lecture} now shows that there is $\gamma \in \Z_p$ such that $\gamma^2 = x^3 + a_2 x^2 + a_4 x + a_6$ and so $(x, \gamma) \in E(\Z_p)$.
...
\begin{proof}
    ...
    However, note that $\F_p$ is perfect, so $(x - \alpha)^2(x - \beta)$ cannot be irreducible over $\F_p$, otherwise $\F_p[x] / \langle(x - \alpha)^2(x - \beta)\rangle$ would be a non-separable finite field extension of $\F_p$.
...
\begin{example}[1(iv) - second example]
    Assume we want to find a curve $E: y^2 = f(x) := x^3 + A x + B$ such that $\tilde{E}(\F_7)$ contains no point $(x, y)$ with $y \neq 0$.
    By considering a scaling isomorphism of the form
    \begin{equation*}
        E \to E', \quad (x, y) \mapsto (u^2 x, u^3 y)
    \end{equation*}
    we can assume wlog that $A \equiv 0, 1, 3 \mod 7$ (as the fourth powers in $\F_7^*$ are $1, 4, 2$).
    Furthermore have
    \begin{equation*}
    ...
    \end{equation*}
    Trying all of them, we find that the only solution such that there is no $(x, y) \in \tilde{E}(\F_7)$ with $y \neq 0$ is $A \equiv 0, B \equiv 6$.
    Indeed, have that
    \begin{equation*}
        \tilde{E}(\F_7) = \{ (1, 0), (2, 0), (4, 0), \O \}
    \end{equation*}
    Furthermore, note that
    \begin{align*}
        1^2 &\equiv 3^3 + 6 = 33 \mod 8 \\
        1^2 &\equiv 1^3 + 6 = 7 \mod 3 \\
        2^2 &\equiv 2^3 + 6 = 14 \mod 5
    \end{align*}
    so Hensel's Lemma \cite[Thm 1.15]{lecture} yields points $(x, y) \in E(\Z_2)$ with $|y|_2 = 1$ and $(x', y') \in E(\Z_3)$ with $|y'|_3 = 1$ and $(x'', y'') \in E(\Z_5)$ with $|y''|_5 = 1$.
\end{example}
...
\begin{example}[2(ii)]
    ...
    So the reduction $\tilde{E}$ modulo 7 is an Elliptic Curve with
    \begin{equation*}
        \tilde{E}(\F_7) = \{ (0, 0), (1, 0), (4, 2), (4, 5), (5, 1), (5, 6), (6, 0), \O \}
    \end{equation*}
    Since $E_{\mathrm{tors}}(\Q) \hookrightarrow \tilde{E}(\F_7)$, this leaves only two cases.
    Either the obvious 2-torsion points are all torsion points (i.e. $\#E_{\mathrm{tors}}(\Q) = 4$) or each of the points $\tilde{E}(\F_7)$ lifts to a torsion point (i.e. $\#E_{\mathrm{tors}}(\Q) = 8$).

    Unlike the previous example however, this time the former is the case.
    To see this, we use the Nagell-Lutz theorem \cite[Thm 5.4]{lecture}.
...
\begin{example}
    Let $p$ be a prime.
    We can find an example by taking the set of ``exhaustive cases'' given by $p(-1) = -p$, similar to the proof of Proposition~\ref{prop:subgroup_mod_every_p}.
...
\begin{proof}
    ...
    The line through $\O$ and $(-a_2, 0)$ is given by $x = -a_2$ and its third point of intersection with $E$ is then $(-a_2, a_1a_2 - a_3)$. 
\end{proof}
...
\begin{itemize}
    \item Apply Corollary~\ref{prop:point_normalization} to get an isomorphic curve whose (nontrivial) 3-torsion point is $(0, 0)$ and the tangent is given by $y = 0$.
...
\begin{example}[4(ii) - trivial example]
    Consider the Elliptic Curve $E: y^2 = x(x^2 + 6 \cdot 30^2 x + 30^4)$.
    Clearly $30^4 = 2^4 \cdot 3^4 \cdot 5^4$ has at least three different prime factors.
    To compute the rank of $E$, note that $E$ is isomorphic to $E': y^2 = x(x^2 + 6x + 1)$ (over $\Q$), and so $\mathrm{rank}(E) = \mathrm{rank}(E')$.
    By Example~\ref{ex:4i}, we thus see that $E$ has rank 0.
\end{example}
...
In other words, we want to find curves $E: y^2 = x(x^2 + ax + b)$ such that the equations
\begin{equation*}
    r l^4 + a l^2 m^2 + \frac b r m^4 = n^2 \quad \text{and} \quad r l^4 + a_1 l^2 m^2 + \frac {b_1} r m^4 = n^2
\end{equation*}
obviously have or have no solution.
This condition is quite easy to check with a computer, so let's just use a brute-force approach to find nice curves.
\begin{example}[4(ii) - main example]
    \label{ex:special_curve}
    Consider the curve
    \begin{equation*}
        E: y^2 = x(x^2 + 47 x + 30)
    \end{equation*}
    Note that $30 = 2 \cdot 3 \cdot 5$ has at least 3 different prime factors.
...
\begin{prop}[4(iii)]
    ...
    Furthermore, we have
    \begin{itemize}
        \item If $a \leq 0 \leq b$, then $\mathrm{rank}(E) \leq 2k - 1$
        \item If $a \perp b$ are coprime, then $\mathrm{rank}(E) \leq k$
        \item If $a \perp b$ and $a \leq 0 \leq b$, then $\mathrm{rank}(E) \leq k - 1$
    \end{itemize}
    ...
    ($\Q$-isogenous curves have the same rank, as isogenies are group homomorphisms with finite kernel and there is the ``dual isogeny'', see \cite[Thm III.4.8]{silverman}, \cite[Corollary III.4.9]{silverman} and \cite[Thm III.6.1]{silverman}).
\end{prop}
\begin{proof}
    Use $a_1, b_1, E', \phi, \hat{\phi}, q, \hat{q}$ as in the lecture.
    Let $v$ denote the number of distinct prime factors of $a^2 - 4b = b_1$ and $w$ denote the number of distinct prime factors of $b$.
    ...
    we see that
    \begin{equation}
        \#(E(\Q)/[2]E(\Q)) \leq 2^v \cdot 2^{w + 1} = 2^{v + w + 1} \label{eq:general_bound}
    \end{equation}
    Finally, note that
    \begin{equation*}
        E(\Q)/[2]E(\Q) \cong E_{\mathrm{tors}}(\Q)/[2]E(\Q) \oplus (\Z/2\Z)^{\mathrm{rank}(E)}
    \end{equation*}
    and $\Z/2\Z \hookrightarrow E_{\mathrm{tors}}(\Q)/[2]E(\Z)$ (there is the nontrivial 2-torsion point $(0, 0)$)
    This yields
    \begin{equation*}
        \mathrm{rank}(E) \leq \log_2(\#(E(\Z)/[2]E(\Q)) / 2) \leq \log_2(2^{v + w}) = v + w \leq 2k
    \end{equation*}

    \paragraph{Assume $a \leq 0 \leq b$} Then the equation
    \begin{equation*}
    ...
    Thus
    \begin{equation*}
        \#(E(\Q)/\hat{\phi}(E'(\Q))) \leq 2^w
    \end{equation*}
    As before find then (since $\hat{\phi}$ has a kernel of size 2)
    \begin{equation}
        \#(E(\Q)/[2]E(\Q)) \leq 2^v \cdot 2^w \label{eq:sign_bound}
    \end{equation}
    and again as before
    \begin{equation*}
        \mathrm{rank}(E) \leq \log_2(2^v \cdot 2^w / 2) = v + w - 1 \leq 2k - 1
    \end{equation*}

    \paragraph{Assume $a \perp b$} Then have that $b_1 = (a^2 - 4b) \perp b$ and thus we find that $v + w = k$.
    Equation~\ref{eq:general_bound} is
    \begin{equation*}
        \#(E(\Q)/[2]E(\Q)) \leq 2^v \cdot 2^{w + 1} = 2^{v + w + 1}
    \end{equation*}
    and so it follows
    \begin{equation*}
        \mathrm{rank}(E) \leq \log_2(2^{v + w + 1} / 2) = v + w = k
    \end{equation*}

    \paragraph{Assume $a \perp b$ and $a \leq 0 \leq b$} Now have both Equation~\ref{eq:sign_bound}
    \begin{equation*}
        \#(E(\Q)/[2]E(\Q)) \leq 2^v \cdot 2^w = 2^{v + w}
    \end{equation*}
    and $v + w = k$.
    As before, it follows
    \begin{equation*}
        \mathrm{rank}(E) \leq \log_2(2^{v + w} / 2) = v + w - 1 = k - 1
    \end{equation*}
\end{proof}
Now we want to examine if the above inequalities are sharp.
\begin{example}
    Consider the curve $E: y^2 = x(x^2 - 6x + 1)$, which satisfies $a \leq 0 \leq b$ and $a \perp b$.
...
\begin{equation*}
    E': y^2 = x(x - 92 x + 2089)
\end{equation*}
satisfies $a \leq 0 \leq b$ and $a \perp b$, and indeed its rank is $\mathrm{rank}(E') = 3 = 4 - 1$ (note that $2089 \cdot 30$ has exactly $4$ prime factors).
...
\section*{Conclusion}
In this work, we have seen a lot of examples of Elliptic Curves with interesting behavior.
The first question dealt with cases in which there are (almost) no points with coordinates in $\Z_p$ on the curve, for certain $p$.
By Hensel's Lemma, this is very closely related to the $\F_p$-rational points of the reduction of $E$ modulo $p$, and we have seen that this only exhibits special behavior for small primes.

In the second question, we continued to study the reduction modulo $p$ and looked for situations where the group of $\F_p$ rational points in the reduction is strictly larger than the torsion group of $E(\Q)$.
These cases are interesting, as they complicate the computation of the torsion group using the standard techniques.

The third question continued to deal with rational torsion points, but now we wanted to find Elliptic Curves with torsion points of specified order.
As it turns out, in some cases we have found a family of defining equations that completely characterize these Elliptic Curves.
Together, we note that when it comes to the torsion group, we find a lot of nice connections and can make definite statements about its structure.
All in all, we understand the torsion part of $E(\Q)$ quite well.

The last question however deals with the free part of $E(\Q)$ (in particular the rank of $E$), which is much more mysterious.
Our main obstacle is that the naive 2-descent method from the lecture only works in a few cases, and in other cases, it is extremely difficult to compute the rank.
We also could not find nice characterizations of whenever the method works.
However, working around this obstacle, we still could find some interesting examples and find their rank.
Furthermore, we present a sharp bound on the rank, which only depends on the coefficients of the Elliptic Curve in a simple way.

Of course, when it comes to the rank, there are still a lot of open questions, and we just touched the topic on the surface.
For example, I would find it interesting to study whether the fact that $\Q$-isogenies preserve the rank allows to prove more statements or compute the rank in more cases.
Finally, this line of thought also leads into current research, and is even connected to one of the Millenium Problems, namely the Birch and Swinnerton-Dyer Conjecture.
